...
setup_config() {
    if [[ -f "${WORKDIR}/.env" ]]; then
        ...
        if [[ "$modify_choice" == "y" || "$modify_choice" == "Y" ]]; then
            ...
            [[ -n "$new_nz_server" ]] && export NEZHA_SERVER="$new_nz_server"

            if [[ -n "$NEZHA_SERVER" ]]; then
                reading "请输入哪吒探针端口 (直接回车保留原配置): " new_nz_port
                [[ -n "$new_nz_port" ]] && export NEZHA_PORT="$new_nz_port"

                reading "请输入哪吒探针密钥 (直接回车保留原配置): " new_nz_key
                [[ -n "$new_nz_key" ]] && export NEZHA_KEY="$new_nz_key"
            fi

            reading "请输入 Argo 固定隧道域名 (直接回车保留原配置): " new_argo_domain
            [[ -n "$new_argo_domain" ]] && export ARGO_DOMAIN="$new_argo_domain"

            if [[ -n "$ARGO_DOMAIN" ]]; then
                reading "请输入 Argo 固定隧道密钥(Json或Token) (直接回车保留原配置): " new_argo_auth
                [[ -n "$new_argo_auth" ]] && export ARGO_AUTH="$new_argo_auth"
            fi
            green "\n配置已更新！"
        else
            green "已选择保持原配置不修改进行部署。"
        fi
        generate_argo_files
    else
        # 初次安装流程
