...
check_port () {
    ...
    tcp_count=$(echo "$tcp_ports" | grep -c .)
    udp_count=$(echo "$udp_ports" | grep -c .)

    if [[ $tcp_count -ne 2 || $udp_count -ne 1 ]]; then

        red "端口规则不符合要求，需要2个TCP端口和1个UDP端口，正在调整..."

        # 删除多余TCP
        if [[ $tcp_count -gt 2 ]]; then
            delete_num=$((tcp_count-2))

            echo "$port_list" | awk '/tcp/ {print $1,$2}' | head -n $delete_num |
            while read port type
            do
        ...
        if [[ $udp_count -gt 1 ]]; then
            delete_num=$((udp_count-1))

            echo "$port_list" | awk '/udp/ {print $1,$2}' | head -n $delete_num |
            while read port type
            do
                devil port del $type $port >/dev/null 2>&1
                green "已删除UDP端口: $port"
        ...
        # 添加TCP到2个
        while [[ $tcp_count -lt 2 ]]
        do
            new_port=$(shuf -i 10000-65535 -n 1)

            result=$(devil port add tcp $new_port 2>&1)

            if [[ $result == *"Ok"* ]]; then
                green "已添加TCP端口: $new_port"
                tcp_count=$((tcp_count+1))
            else
                yellow "TCP端口 $new_port 不可用"
            fi
        done


        # 添加UDP到1个
        while [[ $udp_count -lt 1 ]]
        do
            new_port=$(shuf -i 10000-65535 -n 1)

            result=$(devil port add udp $new_port 2>&1)

            if [[ $result == *"Ok"* ]]; then
                green "已添加UDP端口: $new_port"
                udp_count=$((udp_count+1))
            else
                yellow "UDP端口 $new_port 不可用"
            fi
        done


        green "端口调整完成，请重新连接SSH重新运行脚本"
    ...
    purple "socks5使用TCP端口: $tcp_port2"
    purple "hysteria2使用UDP端口: $udp_port1"


    export ARGO_PORT=$tcp_port1
    export S5_PORT=$tcp_port2
...
read_variables() {
    ...
    if [[ -n "$OLD_UUID" ]]; then
        yellow "\n═══════════════════════════════════"
        yellow "检测到已有安装配置："
        echo -e "${green}  UUID: ${purple}${OLD_UUID}${re}"
        echo -e "${green}  订阅路径: ${purple}${OLD_SUB_PATH}${re}"
        yellow "═══════════════════════════════════"
        
        # UUID 和订阅路径选择
        reading "是否沿用上次的UUID和订阅路径？\n直接回车=沿用 | y=沿用 | n=重新设置【y/n】: " reuse_choice
        
        if [[ "$reuse_choice" != "n" && "$reuse_choice" != "N" ]]; then
            UUID="$OLD_UUID"
            SUB_PATH="$OLD_SUB_PATH"
            green "✓ 已沿用上次的UUID和订阅路径"
        else
            # 询问是否保留原路径
            reading "是否保留原订阅路径 ${OLD_SUB_PATH}？\n直接回车=保留 | y=保留 | n=设置新路径【y/n】: " keep_path
            if [[ "$keep_path" != "n" && "$keep_path" != "N" ]]; then
                # 保留原路径，只修改UUID
                reading "请输入新的UUID (直接回车将自动生成): " new_uuid
                UUID="${new_uuid:-$(uuidgen -r)}"
                SUB_PATH="$OLD_SUB_PATH"
                green "✓ 已保留原订阅路径: ${SUB_PATH}"
                green "✓ 已设置新UUID: ${UUID}"
            else
                # 完全重新设置
                reading "请输入新的UUID (直接回车将自动生成): " new_uuid
                UUID="${new_uuid:-$(uuidgen -r)}"
                reading "请输入新的订阅路径 (直接回车将使用UUID前8位): " new_subpath
                SUB_PATH="${new_subpath:-${UUID:0:8}}"
                green "✓ 已设置新的UUID和订阅路径"
            fi
        fi
    else
        # 首次安装
        echo -e "\n${yellow}首次安装，请配置UUID和订阅路径${re}"
        reading "请输入UUID (直接回车将自动生成): " new_uuid
        UUID="${new_uuid:-$(uuidgen -r)}"
        reading "请输入订阅路径 (直接回车将使用UUID前8位): " new_subpath
        SUB_PATH="${new_subpath:-${UUID:0:8}}"
        ...
        green "  UUID: ${UUID}"
        green "  订阅路径: ${SUB_PATH}"
        reading "确认以上配置？直接回车确认，输入n重新设置【y/n】: " confirm
    ...
    green "  订阅路径: ${purple}${SUB_PATH}${re}"
    green "  订阅链接: ${purple}https://${USERNAME}.${CURRENT_DOMAIN}/${SUB_PATH}${re}\n"
...
install_singbox() {
    ...
    echo -e "${yellow}本脚本同时四协议共存${purple}(vmess-ws,vmess-ws-tls(argo),hysteria2,scoks5)${re}"
